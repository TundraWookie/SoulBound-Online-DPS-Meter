...
- Boss raids such as Spectra Lair require a genuine non-snapshot `bossraid` encounter start followed by its completion event. Logs that attach after the encounter began are excluded because their clock covers only the observed tail of the raid. A party record must also be submitted by a member who remained alive through the ending, because a dead client receives the same ambiguous encounter-end event after either a clear or a wipe.
...
- Spectra Normal and Hard are identified from their invariant incoming `raw_amount` attack signatures (50/80/150 for Normal and 75/240/250 for Hard), so party clears are separated correctly despite mitigation. A Solo record additionally requires personal damage equal to the mode's boss HP: 2.5 million for Normal or 8 million for Hard.
